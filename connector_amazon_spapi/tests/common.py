...
class CommonConnectorAmazonSpapi(TransactionComponentCase):
    ...
    @classmethod
    def load_amazon_spec_fixture(cls, operation_id, response_code="200"):
        """Load sample responses from Amazon SP-API Feeds specification.

        Args:
            operation_id: The API operation (e.g., 'createFeedDocument', 'createFeed')
            response_code: HTTP response code to get example for (default: '200')

        Returns:
            dict: Sample response from the official API spec
        """
        spec_path = os.path.join(os.path.dirname(__file__), "feeds_2021-06-30.json")
            ...
            for _method, operation in methods.items():
                if operation.get("operationId") == operation_id:
                    ...
                    if response_code in responses:
                        # Try to get sandbox example first
                        sandbox = responses[response_code].get("x-amzn-api-sandbox")
                        if sandbox and "static" in sandbox and sandbox["static"]:
                            return sandbox["static"][0].get("response", {})
