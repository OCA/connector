...
class ConnectorCheckpoint(models.Model):
    ...
    def _search_record(self, operator, value):
        ...
        for model in models:
            ...
            checks = self.search(
                [("model_id", "=", model_id), ("record_id", "in", res_ids)]
            )
    ...
    @api.model
    def create_from_name(self, model_name, record_id, backend_model_name, backend_id):
        model_model = self.env["ir.model"]
        ...
        return self.create(
            {"model_id": model.id, "record_id": record_id, "backend_id": backend}
        )
...
def add_checkpoint(env, model_name, record_id, backend_model_name, backend_id):
    checkpoint_model = env["connector.checkpoint"]
    return checkpoint_model.create_from_name(
        model_name, record_id, backend_model_name, backend_id
    )
...
class ConnectorCheckpointReview(models.TransientModel):
    ...
    def _get_checkpoint_ids(self):
        ...
        if context.get("active_model") == "connector.checkpoint" and context.get(
            "active_ids"
        ):
