...
class TestFeedLifecycle(CommonConnectorAmazonSpapi):
    ...
    def test_submit_feed_happy_path(self, mock_call_api, mock_requests_put):
        ...
        # Use realistic responses from Amazon SP-API spec
        # Step 1: Create feed document response
        url = (
            "https://tortuga.s3.amazonaws.com/2019-01-01/"
            "0000000000000/3d4e42b5-1d6e-44e8-a89c-2abfca0625bb"
        )
        ...
        self.assertEqual(feed.external_feed_id, "29323828293")
        ...
        # Check positional args: (method, endpoint, ...)
        self.assertEqual(first_call[0][0], "POST")
        self.assertEqual(first_call[0][1], "/feeds/2021-06-30/documents")
        # Check keyword args for json_data payload
        self.assertEqual(
        ...
        self.assertIn("tortuga.s3.amazonaws.com", upload_call[0][0])
        ...
        self.assertEqual(second_call[0][1], "/feeds/2021-06-30/feeds")
        # Check keyword args for json_data payload
        self.assertEqual(
    ...
    def test_check_feed_status_in_progress(self, mock_call_api):
        ...
        self.assertIn("TEST_FEED_456", call_args[0][1])
    ...
    def test_check_feed_status_in_queue(self, mock_call_api):
        """Test status check when feed is queued (not yet processing)."""
        # Mock IN_QUEUE status from Amazon spec
        mock_call_api.return_value = {
            "feedId": "TEST_FEED_456",
        ...
        # IN_QUEUE should map to 'queued' state
    ...
    def test_check_feed_status_api_error(self, mock_call_api):
        ...
        # check_feed_status should catch exceptions and log the error
        with self.assertLogs(
            "odoo.addons.connector_amazon_spapi.models.feed", level="ERROR"
        ) as log_context:
            feed.check_feed_status()

        # Verify error was logged with feed ID
        self.assertTrue(
            any("TEST_FEED_456" in msg for msg in log_context.output),
            "Feed ID should be in error log",
        )
        self.assertTrue(
            any("Network timeout" in msg for msg in log_context.output),
            "Exception message should be in error log",
        )
    ...
    def test_submit_feed_different_feed_types(self, mock_call_api, mock_requests_put):
        ...
        for feed_type in feed_types:
            # Mock responses for each iteration - use realistic values from spec
            mock_call_api.side_effect = [
                {
                    "feedDocumentId": "3d4e42b5-1d6e-44e8-a89c-2abfca0625bb",
                    "url": "https://tortuga.s3.amazonaws.com/feed/upload",
                },
                {"feedId": "29323828293"},
            ]
